...
if [ -n "$ROOT_AT_1" ]; then # для export_img_bios_gpt и export_img_uefi_gpt
    ...
    # возможно это вызовет проблемы при смене версии grub, так как его часть не обновляется (я все равно не собираюсь менять версию grub в gnubox maker)
    # так же это может вызвать проблему если сменится UUID нового rootfs, который grub возможно использует в EFI разделе для обнаружения rootfs
    boot_dev=""
