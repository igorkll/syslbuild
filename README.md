# syslbuild + gnubox maker + mkbootable 1.10.0
...
* note that when you copy a directory in "directory: items", it is assumed that the target directory to which you are copying files already exists (you are copying To THIS directory (which is indicated by the second element) and NOT itself) if the directory that is indicated as the second (target) in item does not exist, it will be created as chain directories with access rights [0, 0, 700] and not with the access rights that you specified for copying. To avoid this, before copying the item, you can specify that you need to create a target directory and explicitly set access rights
* when copying directories to "directory: items", the rights you specified apply only to items inside the directory (recursively) and not to the target directory. if the target directory does not exist, the permissions will be set as [0, 0, 700] (the same applies when multiple directories are created in a chain. the rights are specified only for the last directory, and all others are created as a chain and get 700 rights)
