...
* as practice shows, using "systemd-nspawn" ("use_systemd_container" in smartchroot) is unreliable and highly dependent on the host system. it is better to use a regular chroot and, if necessary, create symlinks manually instead of systemctl (via "ln -sf" in chroot scripts)
